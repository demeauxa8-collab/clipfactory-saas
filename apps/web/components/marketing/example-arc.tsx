...
export function ExampleArc() {
  ...
  return (
    <div className="rounded-[1.25rem] border border-[var(--color-border)] bg-[var(--color-muted)] p-7">
      ...
      <ol className="mt-6 space-y-3">
        {segments.map((s) => (
          <li
            key={s.role}
            className="flex items-center gap-4 rounded-[0.875rem] border border-[var(--color-border)] bg-[var(--color-background)] p-4"
          >
            <span className="rounded-[0.5rem] bg-[var(--color-foreground)] px-2 py-1 font-mono text-xs tabular-nums text-[var(--color-background)]">
              {s.time}
            </span>
      ...
      <div className="mt-6 flex items-center justify-between rounded-[0.875rem] border border-[var(--color-brand)] bg-[var(--color-brand-soft)]/40 p-4">
        <div>
